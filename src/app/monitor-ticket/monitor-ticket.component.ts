...
@Component({
  ...
  styleUrls: ['./monitor-ticket.component.scss'],
  template: `
        ...
        <ng-container>
          ...
          <div class="bg-white dark:bg-white/10 m-0 p-0 text-theme-gray dark:text-white/60 text-[15px] rounded-10 relative mb-[25px]">
            <div class="pt-[30px] pb-[9px] px-[25px] text-dark dark:text-white/[.87] font-medium text-[17px] flex items-center justify-between max-sm:flex-col max-sm:gap-[15px]">
              <h4 class="mb-0 text-[20px] leading-6 font-medium text-dark dark:text-white/[.87]">Giám sát Ticket</h4>
            </div>
              ...
              <div class="flex items-center justify-center w-full mt-5 mb-[25px] max-md:flex-col max-md:justify-center gap-[15px]">
                <div class="inline-flex items-center flex-wrap w-full gap-[20px] max-md:justify-center">
                  ...
                  <div class="inline-flex items-center">
                    ...
                    >
                      <nz-option *ngFor="let priority of priorityOptions" [nzValue]="priority" [nzLabel]="priority"></nz-option>
                    </nz-select>
                  </div>
                  <div class="inline-flex items-center">
                    <nz-select
...
export class MonitorTicketComponent implements OnInit {
  ...
  private buildTopicFilterGroups(topics: Topic[]): { departmentName: string; topics: string[] }[] {
    ...
    for (const topic of topics) {
      ...
      if (!groups.has(departmentName)) {
        groups.set(departmentName, new Set());
      }
      if (topic.topicName) {
        groups.get(departmentName)?.add(topic.topicName);
      }
    }
    return Array.from(groups.entries())
      .map(([departmentName, topicNames]) => ({
        departmentName,
        topics: Array.from(topicNames).sort((a, b) => a.localeCompare(b))
      }))
  ...
  confirmChangePriority(modalRef?: NzModalRef): void {
    if (this.selectedTicket && this.selectedNewPriority) {
      ...
      if (newCode) {
        ...
        this.selectedTicket.priority = this.getPriorityFromSlaCode(newCode);
      } else {
        this.selectedTicket.slaCode = '';
  ...
  private getPriorityFromSlaCode(slaCode: string | undefined): string {
    return this.slaPriorityByCode[slaCode ?? ''] ?? '';
  }

  private normalizeTicketData(data: any[]): Person[] {
    return data.map((item) => {
      const slaCode = item.slaCode ?? item.SLAcode ?? '';
      return {
        ...item,
        ...
        slaCode,
        priority: this.getPriorityFromSlaCode(slaCode),
